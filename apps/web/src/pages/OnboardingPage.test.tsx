...
test("/onboarding route renders the Welcome to WellnessHub! heading", () => {
  renderViaApp();
...
test("submitting with only Full Name filled (no DOB, no gender) succeeds", async () => {
  ...
  await waitFor(() => expect(mockNavigate).toHaveBeenCalledWith("/devices/pair"));
});
...
test("successful submission sends wellnessPreferences array", async () => {
  const calls: { path: string; opts: RequestInit }[] = [];
  mockApiFetch.implementation = (path, opts) => {
    calls.push({ path, opts });
    return Promise.resolve({});
  ...
  // uncheck "activity" (checked by default), check "weight"
  fireEvent.click(screen.getByLabelText("Daily activity and step tracking"));
  fireEvent.click(screen.getByLabelText("Weight and body composition"));
  ...
  await waitFor(() => expect(calls).toHaveLength(1));
  const body = JSON.parse(calls[0]!.opts.body as string) as Record<string, unknown>;
  const prefs = body["wellnessPreferences"] as string[];
  expect(prefs).toContain("sleep");
  expect(prefs).toContain("weight");
  expect(prefs).not.toContain("activity");
});

// ── Navigation after success ──────────────────────────────────────────────────

test("successful submission navigates to /devices/pair", async () => {
  mockApiFetch.implementation = () => Promise.resolve({});
  renderPage();
...
test("clicking Skip for now navigates to /devices/pair without calling apiFetch", () => {
  const spy = vi.fn(() => Promise.resolve({}));
  mockApiFetch.implementation = spy;
  renderPage();
  ...
  expect(mockNavigate).toHaveBeenCalledWith("/devices/pair");
  expect(spy).not.toHaveBeenCalled();
});
...
// ── Wellness preference checkboxes scope ─────────────────────────────────────
